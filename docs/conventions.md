...
3. **Semantic tokens only.** Components may reference `--sds-color-background-*`, `--sds-color-content-*`, `--sds-color-border-*`, `--sds-space-*`, `--sds-radius-*`, `--sds-typography-*`, `--sds-duration-*`, `--sds-easing-*`, `--sds-elevation-*`. Never a primitive like `--sds-color-brand-600` or `--sds-shadow-*` (use `--sds-elevation-*`), never a raw hex, never a magic pixel value where a token exists. Tokens are edited in `tokens/**/*.json`, never in the generated `src/tokens/*.css`.
