...
3. **Semantic tokens only.** Components may reference `--sds-color-*`, `--sds-space-*`, `--sds-radius-*`, `--sds-font-*`, `--sds-text-*`, `--sds-shadow-*`, `--sds-duration-*`, `--sds-easing-*`, `--sds-elevation-*`. Never a primitive like `--sds-brand-600`, never a raw hex, never a magic pixel value where a token exists. Tokens are edited in `tokens/**/*.json`, never in the generated `src/tokens/*.css`.
